...
export interface PayloadPreview {
  ...
  totalChars: number;
  /** Estimated token count based on character count (~3.5 chars/token). */
...
export function generateVacancyAnalysisPreview(
  ...
  if (input.job.descriptionClean.length > 0) {
    appendStructured(
      included,
      "descriptionClean",
      summarizeText(input.job.descriptionClean),
    );
  ...
  appendStructured(
    included,
    "profileSummary",
    summarizeText(input.profile.summary),
  );
  ...
  appendArrayField(
    included,
    "profileNiceToHaveSkills",
    input.profile.niceToHaveSkills,
  );
  ...
  if (input.resumeHighlights && input.resumeHighlights.length > 0) {
    appendStructured(
      included,
      "resumeHighlights",
      summarizeText(input.resumeHighlights),
    );
...
function label(
  key: string,
  dict: Record<string, string> = ANALYSIS_FIELD_LABELS,
): string {
  return dict[key] ?? key;
}
...
function appendArrayField(
  ...
  target.push({
    ...
    summary:
      values.length > 0
        ? `${values.length} шт.: ${values.slice(0, 5).join(", ")}`
        : "—",
  });
}
