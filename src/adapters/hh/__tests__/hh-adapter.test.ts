  ...
  const doc = {
    ...
    querySelector(selector: string): MockElement | null {
      ...
      for (const [registered, elem] of elements) {
        if (selector === registered || selector.includes(normalizeSelector(registered))) {
          return elem;
        }
      }
      return null;
...
describe('HHAdapter', () => {
  ...
  describe('matchUrl', () => {
    it('returns "vacancy" for HH vacancy URLs', () => {
      expect(adapter.matchUrl('https://hh.ru/vacancy/12345678')).toBe('vacancy');
      expect(adapter.matchUrl('https://spb.hh.ru/vacancy/99999999')).toBe('vacancy');
    });

    it('returns "vacancy" for vacancy URLs with query params', () => {
      expect(adapter.matchUrl('https://hh.ru/vacancy/12345678?query=param')).toBe('vacancy');
      expect(adapter.matchUrl('https://hh.ru/vacancy/12345678?hhtmFrom=vacancy_search_list')).toBe('vacancy');
    });

    it('returns "search" for search URLs', () => {
      expect(adapter.matchUrl('https://hh.ru/search/vacancy')).toBe('search');
      expect(adapter.matchUrl('https://spb.hh.ru/search/vacancy?text=frontend')).toBe('search');
    });

    it('returns "applications" for resume/applicant URLs', () => {
      expect(adapter.matchUrl('https://hh.ru/applicant/resumes')).toBe('applications');
    });
  ...
  describe('extractVacancy — skeleton', () => {
    ...
    it('includes warnings for missing fields', () => {
      ...
      const missingFields = result!.warnings
        ...
        .map((w) => w.field);
      // Should warn about missing title, company, etc.
...
describe('selectors-v1', () => {
  it('has selectors for all expected fields', () => {
    ...
    for (const field of expectedFields) {
      ...
      expect(Array.isArray(SELECTORS_V1[field as keyof typeof SELECTORS_V1])).toBe(true);
      expect(SELECTORS_V1[field as keyof typeof SELECTORS_V1].length).toBeGreaterThan(0);
    }
  });
  ...
  it('prefers data-qa selectors as first fallback', () => {
    ...
    for (const field of Object.keys(SELECTORS_V1) as Array<keyof typeof SELECTORS_V1>) {
